...
F-004 repair: Replaced inferred/generic profiler attribution with observed Metal command-buffer labels, observed combined B2 ordered encoder labels, observed C1 native-stem labels, command-buffer-ID joins, 100 nonzero GPU rows joined through 100 observed submission mappings, source-export hashes, and clean completion. The checker derives path cardinality from event rows and runs negative blank-row, truncation, wrong-path, and schema-only tests.
