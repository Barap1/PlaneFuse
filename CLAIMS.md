...
Claim wording: "The bounded Release R6.5 experiment, generated by committed harness df3dccf, used a 32-frame, source-resolution 1920x1080 NV12 replay, five alternating 200-pair batches, and the same explicit .all Core ML tail. The fair direct camera-space B materialized planar Float32 RGB and ran the unchanged B2 stem in one ordered submission; direct camera-space C eliminated the resized NV12 intermediate and ran the unchanged native stem in one ordered submission. Correctness passed: accepted-path activation errors were 0, direct B/C activation max error was 8.344651e-6, top-1 agreement was 1.0, and no task disagreements were recorded. Direct B p50 post-input-to-result was 1.6945 ms and direct C p50 was 2.3822 ms; paired B-minus-C p50 was -1.4763 ms with median bootstrap 95% CI [-1.5071, -1.4393] ms. C was slower, so no camera-space fusion speedup claim is made and accepted C1 is retained for R7."

Evidence files: `proof/r6.5-camera-space.json`, `proof/r6.5-camera-source-replay.manifest.json`, `proof/r6.5-camera-source-replay.bin`, `proof/r6.5-camera-space-release.log`, `scripts/check_r6_5_camera_space_artifact.py`, `proof/reviews/R6.5-CANDIDATE-B01B3E1-20260810-SOL-03.md`
...
Accepted status: pending final SHIP re-review after Sol FIX-FIRST finding F-003; measurement generated by df3dccf and promoted into the evidence commit
