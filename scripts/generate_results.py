...
C1-SR is {final['aggregate']['c1_source_reuse_percentage_vs_b2']:.1f}% lower than B2 by marginal p50 while retaining the same pretrained tail and passing the declared quality checks.
...
C1-SR is {final['aggregate']['c1_source_reuse_percentage_vs_b2']:.1f}% lower than B2 and {final['aggregate']['c1_source_reuse_percentage_vs_c1']:.1f}% lower than C1 by marginal p50. The paired B2 minus C1-SR median 95% confidence interval for the absolute latency difference is `[{ci[0]:.6f}, {ci[1]:.6f}] ms`. These are different estimands; the paired interval is not a confidence interval for the percentage reduction.
...
- Read-only, model-based GPT-5.6 Sol adversarial review (not an external human audit): **{review_status}**, with no open method findings.
...
frozen reviewed result remains the {final['aggregate']['c1_source_reuse_percentage_vs_b2']:.4f}% headline experiment.
